...
folder("CCPLSolutions/PrePipelines") {
    pipelineJob('CCPLSolutions/PrePipelines/ReleaseMergeEvent') {
        description('')
        parameters {
        ...
        triggers {
            genericTrigger {
                ...
                token('prePipelineReleaseMergeEvent')
                regexpFilterText('$ref $commits')
                regexpFilterExpression('^refs\\/heads\\/(re-v*).*\\[\\{.*$')
            ...
            cpsScm {
                scm {
                    git {
                        ...
                        branches('*/master')
                        scriptPath('projects/prePipelines/ReleaseMergeEvent/Jenkinsfile')
                        extensions {}
                    }
                }
                lightweight(true)
